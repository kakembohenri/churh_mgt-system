...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
	...
	$confirm = md5($_POST['retype_password']);

	if($new != $confirm){
	 $_SESSION['error'] = 'Passwords dont match';
	}else{
		// Check table to see if passwords match
		$query = mysqli_query($conn, "select * from users where id='$sys_user' and password='$current'") or die(mysqli_error($conn));
		$count = mysqli_num_rows($query);

		if(!$count > 0){
			$_SESSION['error'] = 'Password for this user is incorrect!';
		}else{
			
			// Update passwords
			$query = mysqli_query($conn, "update users set password='$new' where id='$sys_user'") or die(mysqli_error($conn));
					...
					<div id="block_bg" class="block">
						...
						<div class="block-content collapse in">
							<div class="span12">
								...
								<?php
								// $query = mysqli_query($conn,"select * from admin where admin_id = '$church_id'")or die(mysqli_error());
...
<?php include('subscription/reset.php');?>
</html>